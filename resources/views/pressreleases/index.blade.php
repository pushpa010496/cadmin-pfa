...
@section('header')
    <div class="page-header clearfix">
        <h3>
            ...
            <a class="btn btn-sm btn-success pull-right" href="{{ route('pressrelease.create') }}"><i class="fa fa-plus-square-o" aria-hidden="true"></i> Create</a>
        </h3>
    </div>
...
@section('content')
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
               {!! Form::open(['method'=>'GET','url'=>'pressrelease','role'=>'search'])  !!}
                        ...
                        @foreach($pressreleases as $value)
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    <a class="btn btn-sm btn-success" href="{{ url('pressrelease/reactivate', $value->id) }}">
                                         Active ?
                                    </a>

                                    <a class="btn btn-sm btn-primary" href="{{ route('pressrelease.show', $value->id) }}">
                                        <i class="fa fa-eye" aria-hidden="true"></i> View
                                    </a>
                                    
                                    <a class="btn btn-sm btn-info" href="{{ route('pressrelease.edit', $value->id) }}"><i class="fa fa-pencil-square" aria-hidden="true"></i> Edit
                                    </a>
                                      @if($value->active_flag == 1 )
                                    <form action="{{ route('pressrelease.destroy', $value->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Delete? Are you sure?');">
                                        {{csrf_field()}}
